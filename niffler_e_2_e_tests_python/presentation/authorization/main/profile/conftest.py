...
from niffler_e_2_e_tests_python.configs import GATEWAY_URL, TEST_PASSWORD, TEST_USER, FRONT_URL1
...
@pytest.fixture
def clear_spend_and_category(clear_spend: None, clear_category: None) -> None:
...
def goto_profile(login_page: 'LoginPage', main_page: 'MainPage', presentation_page: 'PresentationPage'):
    ...
    if main_page.driver.locator(main_page.profile).is_visible() and main_page.driver.url != f'{FRONT_URL1}/profile':
        # main_page.goto_url(f'{FRONT_URL1}/profile')

        main_page.click(main_page.profile)
    if presentation_page.driver.url != f'{FRONT_URL1}/profile':
